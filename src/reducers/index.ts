...
import type { State, Action, EDQuery, DataObj } from "../types";
...
function dataReducer(state = InitialState.data, a: Action) {
  ...
  case GET_ASYNC_RESPONSE: {
    ...
    case SET_MENU_DATA: {
      const nestedData = rollup(
        ...
        // year = synthetic variable for organizing the menu
        (d: any) => d.event.slice(-4),
        (d: any) => d.event,
        (d: any) => d.office,
        //@ts-ignore
    ...
    case ADD_PCT_DATA: {
      ...
      const nextQueryData = a.payload.data.map(
        ({ ad, ed, tally, sum_tally, tally_pct }: DataObj) => ({
          ad,
          ed,
          tally,
          sum_tally: +sum_tally,
      ...
      const nextQueryLookup = getLookupFromQueries(nextQueries);
      return {
        ...state,
        queries: nextQueries,
        lookup: nextQueryLookup
      };
    }
    case SET_ADED_DATA: {
  ...
  case SET_QUERY_PROP: {
    ...
    nextQuery.complete = isQueryComplete(nextQuery);

    // if there is only one next dropdown option available, select it
    // and repeat until the menu has multiple options
    // the last level, Candidate, should implicitly have multiple options
